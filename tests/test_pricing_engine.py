...
class TestCalculateDistanceCost:
    ...
    def test_tier_1_to_3km(self):
        # 2.5 km = 3500 + (1.5 * 1000) = 5000
        assert calculate_distance_cost(2.5, BASE_CONFIG) == 5000

    def test_tier_1_exact_3km(self):
        # 3.0 km = 3500 + (2.0 * 1000) = 5500
        assert calculate_distance_cost(3.0, BASE_CONFIG) == 5500

    def test_tier_3_to_5km(self):
        # 4.0 km = 3500 + (2.0 * 1000) + (1.0 * 800) = 6300
        assert calculate_distance_cost(4.0, BASE_CONFIG) == 6300

    def test_tier_3_exact_5km(self):
        # 5.0 km = 3500 + 2000 + 1600 = 7100
        assert calculate_distance_cost(5.0, BASE_CONFIG) == 7100

    def test_tier_5plus(self):
        # 7.0 km = 3500 + 2000 + 1600 + (2.0 * 700) = 7100 + 1400 = 8500
        assert calculate_distance_cost(7.0, BASE_CONFIG) == 8500

    def test_rounding_to_nearest_100(self):
        # 1.15 km = 3500 + (0.15 * 1000) = 3650 -> rounded to 3700
        assert calculate_distance_cost(1.15, BASE_CONFIG) == 3700
...
class TestCalculateFullPrice:
    ...
    async def test_mixed_distance_and_fixed_segments(self, config_rows, tool_rows):
        ...
        assert result["route_cost"] == 3500
        assert result["stop_fee"] == 0  # only 1 distance segment
        assert result["segment_fixed_prices"] == 8000
        assert result["total"] == 11500
    ...
    async def test_acompanante_doubles(self, config_rows, tool_rows):
        ...
        assert result["total"] == 7000  # 3500 * 2
    ...
    async def test_tools_surcharge(self, config_rows, tool_rows):
        ...
        assert result["total"] == 5000  # 3500 + 1000 + 500
    ...
    async def test_all_modifiers_stacked(self, config_rows, tool_rows):
        ...
        # route_cost (6300) + stop_fee (1x1500) + segment_fixed (8000) = 15800
        # + canasta (1000) + nequi (500) + rain (1000) = 18300
        # ×2 acompanante = 36600
        assert result["total"] == 36600
